...
/**
 * Die Klasse fügt ein Icon zum System-Tray hinzu.<br>
 * Ueber das Icon kann mit Linksklick der Maus ein Standard-Menue mit Basisfunktionen<br>
 * fuer die Programmsteuerung geoeffnet werden. Ein Rechtklick auf das Icon oeffnet die GUI, die aber separat programmiert werden muss. Das Interface für die Anmeldung am TrayIcon ist noch in Arbeit
 * ...<br>
 * <br>
 * Das Aussehen des TrayIcon kann je nach Status des Backup-Programms veraendert werden.<br>
 * Die zur Verfuegung stehenden Stati sind in den Konstanten STATE_xxx der Klasse festgelegt.<br>
 * <br>
 * Ein StatusListener kann angemeldet werden, so dass die Icons dynymisch auch fuer weitere grafische Komponenten als Icons zur Verfügung gestellt werden.<br>
 * Wenn z.B. ein Fenster minimiert wird, kann dadurch in der Taskleiste der Staus des Programms sichtbar gemacht werden.<br>
 * Der Staus <b>STATE_PERORMING_BACKUP</b> ist animiert, so dass ersichtlich wird, dass das Programm aktiv ist.
 */
...
public class TrayUtility
{
   ...
   private Thread                   imageThread;                                        // Für animiertes Icon
    ...
    * 
    * @param state
    *           The current state. Constant from TrayIconUtil
    */
    ...
    * 
    * @param path
    *           Absolute or relative path for image to be loaded
   ...
   private void createAndShowGUI ()
   {
      ...
      /*
       * trayIcon.addMouseListener ( new MouseListener () { public void mouseReleased ( MouseEvent e ) { } public void mousePressed ( MouseEvent e ) { } public void mouseExited ( MouseEvent e ) { }
       * public void mouseEntered ( MouseEvent e ) { } public void mouseClicked ( MouseEvent e ) { if ( e.getButton () == MouseEvent.BUTTON1 ) // Wenn linker Maus-Button ... { try // ... dann
       * Rechtsklick an der gleichen Stelle ausloesen { Robot robot = new Robot (); robot.mousePress ( InputEvent.BUTTON3_DOWN_MASK ); robot.mouseRelease ( InputEvent.BUTTON3_DOWN_MASK ); } catch (
       * Exception e1 ) { e1.printStackTrace (); } } } } );
       */

      aboutItem.addActionListener ( e -> {
         JOptionPane.showMessageDialog ( null, "<html><H1>BK-Backup</h1>\n" + "Version 0.1.01\n" + "Berufskolleg Alsdorf\n" + "52477 Alsdorf, Heidweg 2\n" + "<html><b>www.bk-alsdorf.de</b>\n" + "\u00A9 21FIA1", "BK-Backup", JOptionPane.INFORMATION_MESSAGE );
      } );
   ...
   /*
    * Set Look & Feel to current system style. Will be "Metal", if no system style is supported.
    */
